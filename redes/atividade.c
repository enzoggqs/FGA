...
int main(int argc,char **argv) {
  ...
  end_web.sin_port 	   = ntohs(8001);
  const char* ip = "127.0.0.1";
    if (inet_pton(AF_INET, ip, &(end_web.sin_addr)) <= 0) {
        fprintf(stderr, "Erro ao converter o endereço IP\n");
        return 1;
    }
  printf("Endereço IP: %s\n", inet_ntoa(end_web.sin_addr));
